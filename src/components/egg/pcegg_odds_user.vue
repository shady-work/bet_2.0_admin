      ...
      <tr v-for="(v,k) in list">
        ...
        <td>
          <div v-for="(val,key) in all_handicap" class="pull-left ml10">
            <div class="hide">
              {{v.egg_ratelist[key]?v.egg_ratelist[key]:(v.egg_ratelist[key]={})}}
            </div>
            <input type="checkbox"   v-model="list[k].egg_ratelist[key].ratewin_name" v-bind:value="val"   >
  ...
  export default {
    ...
    methods: {
          ...
          .then(function (res) {
            ...
            if (res.data.status == 200) {
              ...
              for(let i = 0 ; i<this.list.length;i++)
              {
                ...
                {
                  if(this.list[i].egg_ratelist[j] && this.list[i].egg_ratelist[j].ratewin_name)
                  {
                    this.list[i].oldpk.push(this.list[i].egg_ratelist[j].ratewin_name);
                  }
                  else
                  {
      ...
      change_handicap:function(user_id,index){
        ...
        for(let i = 0;i<this.list[index].egg_ratelist.length;i++)
        {

          if(this.list[index].oldpk[i])
          {
            if(this.list[index].egg_ratelist[i].ratewin_name === false)
            {
              this.delete_handicap(this.list[index].egg_ratelist[i].id);
            }
          }
          if(this.list[index].egg_ratelist[i].ratewin_name === true)
          {
            if(this.list[index].oldpk[i])
            {
              console.log('以前就有的');
      ...
      delete_handicap:function(handicap_id){
        this.$http.delete(`${this.api}/admin/egg/ratelist/${handicap_id}`)
          .then(function(res)
          {
            console.log(res.data);
            if(res.data.status == 200)
