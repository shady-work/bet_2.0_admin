    ...
    <div class="personal-handicaps">
        <div class="left">
            ...
            <div class="choose_lty">
                <h4 class="text-center" style="margin-bottom: 14px;">PC蛋蛋</h4>
            </div>
            <!--choose_handicap-->
        ...
        <div class="right">
            <h4 style="margin-top: 8px;margin-bottom: 14px;">
                ...
                <router-link to="pcegg" class="pull-right mr10">返回查看PC蛋蛋用户信息</router-link>
            </h4>
            ...
            <div class="cqssc" >
                ...
                <table class="table table-hovor table-bordered table-striped text-center">
                    ...
                    <tr >
                        <td width="70">特码/球序</td>
                        <td v-for="(v,k,index) in odds_ssc_one[4]" v-if='index>=20&&index<28'>特码{{index}}</td>
                        <td>单注最小限额</td>
                        <td>单注最大限额</td>
                        <td>单期限额</td>
                    ...
                    <tr>
                        <td>赔率</td>
                        <td v-for="(v,k,index) in odds_ssc_one[4]" v-if='index>=20&&index<28'>
                            <input type="text" class="text-center form-control" v-model="odds_ssc_one[4]['e' + (index+1)]" style="padding:0px 0px;">
...
<script>
    export default {
        ...
        methods:
            {
                 ...
                 * 获取用户的egg盘口
                 */
                get_systems_handicaps: function (type = 'egg')
                {
                    this.$http.get(`${this.api}/admin/egg/odds?user_id=${this.u_id}`)
                        .then(function (res)
                        {
                    ...
                    this.$http.get(`${this.api}/admin/lotteryList/${this.u_id}`)
                        .then(function (res)
                        {
                            if (res.data.status == 200)
                            {
                                ...
                                //查看用户是否有cqssc这个彩种
                                if(this.users_lties.indexOf('pcegg') == -1)
                                {
                                    this.$router.go(-1)
                                }
                                else
                ...
                custom_handicap: function ()
                {
                    ...
                    data.table = 'egg' + "_" + this.handicaps[this.index];
                    data.odds = this.odds_ssc_one;
                    this.$http.post(`${this.api}/admin/egg/customOdds`, data)
                        .then(function (res) {
                            // console.log(res.data);
        ...
        created() {
            //获取用户的id
            this.u_id = this.$route.params.u_id ? this.$route.params.u_id : window.sessionStorage.handicaps_uid_egg;
            window.sessionStorage.handicaps_uid_egg = this.u_id;
            this.get_users_lties();
            this.get_user_info();
